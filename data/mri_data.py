...
class DataTransform:
    ...
    def __call__(self, kspace, mask, target, attrs, fname, slice):
        ...
        """
        kspace = transforms.to_tensor(kspace) # rows, cols, 2 [We are reading only one slice k-space single coil data]
        ...
        if self.mask_func:
            ...
            masked_kspace = kspace  # rows, cols, 2

        # Inverse Fourier Transform to get zero filled solution
        image = transforms.ifft2(masked_kspace) # this is also complex. rows, cols, 2

        target = transforms.ifft2(kspace) # rows, cols, 2
        # Absolute value
        abs_image = transforms.complex_abs(image)  # rows, cols
